...
bool Obj::isOutsideRoom(Sides side)
{
	...
	case RIGHT:
		...
		break;
	case UP:
		if(y - sprite->origY + sprite->image->getSize().y <= 0)
		{
			return true;
		}
		return false;
		break;
	case LEFT:
		if(x - sprite->origX + sprite->image->getSize().x <= 0)
		{
			return true;
		}
		return false;
...
Obj* Obj::getCollisionAt(double x, double y, ObjectType type)
{
	for(unsigned int i=0; i<HGame::getObjectList().size(); i++)
	{
		...
		if(o->objectType == type)
		{
            ...
            {
				for(int sx = static_cast<int>(overlap.left); sx < overlap.left + overlap.width; sx++)
                {
					for(int sy = static_cast<int>(overlap.top); sy < overlap.top + overlap.height; sy++)
                    {
